    ...
    if (redraw_menu && al_is_event_queue_empty(event_queue)) {
        ...
        if (tabela_visible_menu) {
            al_draw_scaled_bitmap(tabela_menu, 0, 0, al_get_bitmap_width(tabela_menu), al_get_bitmap_height(tabela_menu),
                80, 30, al_get_bitmap_width(tabela_menu) * 1, al_get_bitmap_height(tabela_menu) * 1, 0);
        }
        else if (text_visible_menu) {
            al_draw_text(font_menu, al_map_rgb(0, 0, 0), 10, 417 - 50, ALLEGRO_ALIGN_LEFT, "Use a tecla BACKSPACE para iniciar");
            al_draw_text(font_menu, al_map_rgb(0, 0, 0), 10, 417 - 25, ALLEGRO_ALIGN_LEFT, "Use a tecla TAB para ver a tabela de elementos");
        }
    }
}
